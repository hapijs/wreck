...
var internals = {
    defaults: {
        keepAlive: true,
        maxFreeSockets: 256,
        freeSocketsTimeout: 2 * 60 * 1000                   // 2 minutes
    }
};
...
internals.HttpAgent.prototype.free = function (socket, host, port, localAddress) {
    ...
    if (this.isSocketUsable(socket)) {
        ...
        if (this._idleSockets[name].length < this.options.maxFreeSockets) {
            ...
            if (this.options.freeSocketsTimeout) {
                socket.setTimeout(this.options.freeSocketsTimeout, internals.destroySocket, socket);
            }
            else {
                socket.setTimeout(0);
            }
...
internals.destroySocket = function () {
