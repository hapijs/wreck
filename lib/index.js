...
internals.resolveUrl = function (baseUrl, path) {
    ...
    if (!parsedBase.search) {
        ...
        var baseQuery = QueryString.parse(parsedBase.query);
        var pathQuery = QueryString.parse(parsedPath.query);
        var mergedQuery = Hoek.applyToDefaults(baseQuery, pathQuery);
        parsedBase.search = QueryString.stringify(mergedQuery);
    }
