...
exports.request = function (method, url, options, callback, _trace) {
    ...
    Hoek.assert(options.payload === null || options.payload === undefined || typeof options.payload === 'string' || options.payload instanceof Stream || Buffer.isBuffer(options.payload), 'options.payload must be either a string, Buffer, or Stream');
    ...
    var onDownstreamFinish = null;
    var onDownstreamError = null;
    ...
    var finish = function (err, res) {
        ...
        if (!isFinished) {
            ...
            req.on('error', internals.ignore);
            clearTimeout(timeoutId);

            if (options.downstreamRes) {
                options.downstreamRes.removeListener('finish', onDownstreamFinish);
                options.downstreamRes.removeListener('close', onDownstreamError);
            }
...
Util.inherits(internals.Collector, Stream.Writable);
