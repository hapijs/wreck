...
internals.Client.prototype.request = function (method, url, options, callback, _trace) {
    ...
    const onResponse = (res) => {
        ...
        const redirectReq = this.request(redirectMethod, location, redirectOptions, finish, _trace);
...
internals.Client.prototype.read = function (res, options, callback) {
    ...
    let finish = function (err, buffer) {
    ...
    const onResError = function (err) {

        return finish(Boom.internal('Payload stream error', err));
    };

    const onResClose = function () {
    ...
    const onReaderError = function (err) {
