...
exports.request = function (method, url, options, callback, _trace) {
    ...
    if (uri.agent && options.maxSockets !== undefined) {
        uri.agent.maxSockets = options.maxSockets;
    }
    // Only override when the defaults are used on the agent
    else if (uri.agent && Http.Agent.defaultMaxSockets === uri.agent.maxSockets) {
        uri.agent.maxSockets = Infinity;
    }
