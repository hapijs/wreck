...
exports.request = function (method, url, options, callback, _trace) {
    ...
    _trace = (_trace || []);
    _trace.push({ method: uri.method, url: url, time: now });
    ...
    var finish = function (err, res) {
        ...
        exports.log(err, req, res, _trace);
...
// Logging for debug support

exports.log = function (err, req, res, trace) {

    if (!process.env.WRECK_DEBUG_CONSOLE && !process.env.WRECK_DEBUG_FILE) {
        return;
    }
    ...
    if (process.env.WRECK_DEBUG_FILE) {
        internals.logFile = Fs.createWriteStream(process.env.WRECK_DEBUG_FILE);
        log.pipe(internals.logFile);
    }
...
internals.tryParseBuffer = function (buffer) {
    ...
    try {
        result.json = JSON.parse(buffer.toString());
    }
    catch (err) {
        result.err = err;
    }
