...
exports.request = function (method, url, options, callback, _trace) {
    ...
    // a false agent is valid along with rejectUnauthorized as pooling will be disabled with rejectUnauthorized
    Hoek.assert(!options.agent || options.rejectUnauthorized === undefined,
        'options.agent cannot be set to an Agent at the same time as options.rejectUnauthorized being specified');
    ...
    if (options.rejectUnauthorized !== undefined && uri.protocol === 'https:') {
        options.agent = new Https.Agent({ rejectUnauthorized: options.rejectUnauthorized, maxSockets: Infinity })
    }
